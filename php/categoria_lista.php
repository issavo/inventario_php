<?php
    ...
    if(isset($busqueda) && $busqueda != " "){
        $consulta_datos= "SELECT * FROM categoria WHERE categoria_nombre LIKE '%$busqueda%' OR categoria_ubicacion LIKE '%$busqueda%' ORDER BY 
    ...
    if($total >= 1 && $pagina <= $Npaginas){
        ...
        if($total >= 1){
            ...
            $tabla.='
                <tr class="has-text-centered">
                    <td colspan="6">
                        No hay registros en el sistema para mostrar
                    </td>
                </tr>
            ';
        }
